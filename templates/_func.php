...
if($item->hasChildren() && ($item->id != 1)) {
		// markup for the link
		echo "<a class='has-children' href='$item->url'>$item->title</a>";

   } elseif($item->id == 1) {

   echo "<a class='home-page' href='$item->url'>$item->title</a>";

} else {
	echo "<a class='not-has-children' href='$item->url'>$item->title</a>";
}
